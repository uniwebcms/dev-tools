...
const UniwebTeaser = ({ content, params }) => {
  console.log({ params });
  const {
    screenScale = 1,
    screenSpacing = "relaxed",
    showLightEffects = false,
    className = "",
  } = params || {};

  // Screen spacing presets
  const spacingClasses = {
    compact: {
      screen1: "top-2 sm:top-4 left-0",
      screen2: "top-6 sm:top-10 md:top-12",
      screen3: "bottom-2 sm:bottom-4 right-0",
    },
    normal: {
      screen1: "top-4 sm:top-8 left-0",
      screen2: "top-8 sm:top-12 md:top-16",
      screen3: "bottom-4 sm:bottom-8 right-0",
    },
    relaxed: {
      screen1: "top-6 sm:top-12 left-0",
      screen2: "top-12 sm:top-20 md:top-24",
      screen3: "bottom-6 sm:bottom-12 right-0",
    },
  };

  const spacing = spacingClasses[screenSpacing] || spacingClasses.normal;

  // Scale classes for screens
  const getScaleClass = (baseSize) => {
    const scaleFactor = screenScale;
    return {
      width: `${Math.round(baseSize * scaleFactor)}px`,
      height: `${Math.round(baseSize * 0.72 * scaleFactor)}px`, // maintain aspect ratio
    };
  };
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 px-6 py-12 sm:px-8 sm:py-16 lg:px-16 lg:py-20 xl:px-24 xl:py-28 2xl:px-32 2xl:py-32">
        ...
        <div className="relative flex flex-col lg:flex-row items-center gap-12 lg:gap-16 xl:gap-20 2xl:gap-32">
          ...
          <div className="hidden sm:flex flex-1 lg:flex-[1.1] relative h-64 sm:h-72 md:h-80 lg:h-96 xl:h-[420px] w-full perspective-1000">
            {/* Screen 1 - Back left */}
            <div
              className={`absolute ${spacing.screen1} transform -rotate-y-20 rotate-x-5 -translate-z-60 animate-float-slow`}
              style={{
                width: `${220 * screenScale}px`,
                height: `${160 * screenScale}px`,
                "@media (min-width: 640px)": getScaleClass(240),
                "@media (min-width: 768px)": getScaleClass(260),
                "@media (min-width: 1024px)": getScaleClass(320),
                "@media (min-width: 1280px)": getScaleClass(360),
              }}
            >
            ...
            {/* Screen 2 - Center */}
            <div
              className={`absolute ${spacing.screen2} left-1/2 -translate-x-1/2 transform rotate-x-3 translate-z-20 z-30 animate-float-medium`}
              style={{
                width: `${260 * screenScale}px`,
                height: `${180 * screenScale}px`,
                "@media (min-width: 640px)": getScaleClass(280),
                "@media (min-width: 768px)": getScaleClass(300),
                "@media (min-width: 1024px)": getScaleClass(360),
                "@media (min-width: 1280px)": getScaleClass(400),
              }}
            >
              <div className="h-full bg-white/5 backdrop-blur-xl rounded-xl lg:rounded-2xl border border-white/10 shadow-2xl overflow-hidden">
                {/* Minimal browser bar */}
            ...
            {/* Screen 3 - Back right */}
            <div
              className={`absolute ${spacing.screen3} transform rotate-y-20 rotate-x-5 -translate-z-40 z-20 animate-float-slow animation-delay-1000`}
              style={{
                width: `${220 * screenScale}px`,
                height: `${160 * screenScale}px`,
                "@media (min-width: 640px)": getScaleClass(240),
